	...
	public static void downloadSound(String url, String fileName) {
		...
		Executable.executeCommand(
			...
			Executable.YT_DLP.getCommandLine().addArguments(new String[] {
				...
				"--audio-format", "vorbis", "--audio-quality", ClientConfig.get().download.audioBitrate.getValue(),
			...
			}, false).addArguments(ClientConfig.get().download.parameters)
		).subscribe("main")
			.onOutput((output) -> {
	...
	public static void simulateDuration(String url, Consumer<Integer> durationConsumer, Runnable onDone) {
		Executable.executeCommand(
				...
				Executable.YT_DLP.getCommandLine()
						...
						.addArguments(ClientConfig.get().download.parameters))
				.subscribe("duration")
				.onOutput(output -> {
					String type = output.substring(0, output.indexOf(':') + 1);
					String message = output.substring(type.length()).trim();
